...
program
  ...
  .requiredOption('--lang <language>', 'Target language code (e.g., ja, fr, es)')
...
async function extractStrings(srcDir: string, outDir: string, lang: string): Promise<void> {
  ...
  if (fs.existsSync(localeFile)) {
    try {
      const existing = yaml.load(fs.readFileSync(localeFile, 'utf-8')) as Record<string, string>;
...
function extractTemplateStrings(content: string): string[] {
  ...
  while ((match = templateRegex.exec(content)) !== null) {
    ...
    const normalized = templateContent.replace(/\$\{[^}]+\}/g, () => {
      ...
      const placeholderName = placeholderNames[placeholderCount] || `arg${placeholderCount}`;
      placeholderCount++;
